...
const NetState = (() => {
  ...
  function encode(game, seq, events) {
    ...
    const out = {
      ...
      players: game.players.map((p) => [
        ...
        p.stats.goals, p.stats.assists, p.stats.steals, p.stats.saves, Math.round(p.ping), p.exhausted ? 1 : 0, p.dribbleLag,
      ]),
  ...
  function apply(game, s) {
    ...
    s.players.forEach((a, i) => {
      ...
      p.stats.goals = a[k++]; p.stats.assists = a[k++]; p.stats.steals = a[k++]; p.stats.saves = a[k++]; p.ping = a[k++]; p.exhausted = !!a[k++]; p.dribbleLag = a[k++];
    });
