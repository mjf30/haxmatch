...
OLD = dict(SELF=33, BALL=12, MATE=8, OPP=8, GOALS=6, MISC=12, MM=4, MO=5)
...
def mapping():
    m = []   # (old_idx, new_idx)
    o = n = 0
    for k in range(OLD['SELF'] + OLD['BALL']):
        m.append((o + k, n + k))
    o += OLD['SELF'] + OLD['BALL']; n += NEW['SELF'] + NEW['BALL']
    ...
    for k in range(OLD['GOALS'] + OLD['MISC']):
        m.append((o + k, n + k))
    return m
